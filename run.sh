...
if ! [[ "$user_domain" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
    # This regex only checks format, not validity of the IP address
...
echo "Running the application on $user_domain:$user_port..."
docker run -d -p $user_port:3000 -v "$(pwd)/$file_to_mount":/usr/src/app/$file_to_mount rox-api-approval
